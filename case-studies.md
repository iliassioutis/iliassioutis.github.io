...
In parallel with the clinical validation, I assembled a per-device regulatory evidence pack to support Greek-market traceability and App Store review for the connected devices. The pack consolidates the delivery-time documentation used in practice — including the EOF notification/traceability letter (where applicable), manufacturer Declarations of Conformity (and related certificates/transition documentation where relevant), plus device-specific clarification letters prepared for Apple review — so each device model can be traced to its supporting evidence.
...
- **Regulatory device documentation (Greek market traceability):** compiled a per-device evidence pack used during delivery to maintain traceability from device model → supporting documentation (e.g., EOF notification letter where applicable, manufacturer Declarations of Conformity, relevant certificates/transition confirmations, and device-specific clarification letters prepared for App Store review).
- ✅ Artifacts: [Evidence & downloads](/clinical-validation) · [Report (PDF)](/assets/pdfs/epokratis-medaiconnect-validation-report.pdf) · [Repro pack (ZIP)](/assets/downloads/validation_package.zip) · <a href="https://tma.gr/wp-content/uploads/epokratis/regulatory-approval-documents.zip" target="_blank" rel="noopener noreferrer">Regulatory approval documents (ZIP)</a>

**Regulatory device docs (what’s inside):** one folder per device model containing the delivery-time evidence pack used for traceability (e.g., manufacturer Declarations of Conformity and related certificates/transition documentation where relevant), plus device-specific Apple clarification PDFs and the EOF notification/traceability letter included in the ZIP.
...
- **Validation + Quarantine with explicit reasons:** a common layered-pipeline pattern is to keep clean/validated rows in Silver and **quarantine invalid records** for review and auditing.
  Reference: [Databricks — What is the medallion lakehouse architecture?](https://docs.databricks.com/aws/en/lakehouse/medallion) (Example medallion architecture; Silver layer mentions “quarantining invalid records”).

- **Health score (demo metric):** the score here is a simple publishing pattern for a “business-ready metric” (not a machine learning model). In real systems it would be calibrated using **Original Equipment Manufacturer (OEM)** guidance and engineering standards (for example, standards like **ISO 20816-1** that define general procedures for vibration **measurement and evaluation** and discuss **operational limits**), plus historical maintenance outcomes.  
