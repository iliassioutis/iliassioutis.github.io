...
- **Dependency mapping**  
  ...
  - **External services & APIs:** third-party endpoints, rate limits, authentication flows (login/token exchange), **service commitments (SLA — Service Level Agreement, e.g., expected uptime/response times)**, and change-notification channels (how we learn about breaking changes).  
  ...
  - **Data & infrastructure dependencies:** environments (development/test/staging/production), certificates/keys (to secure connections), **feature flags (toggles that turn features on/off per environment or user group without a new release)**, messaging/queues [a reliable “inbox” between systems for events/requests that don’t happen instantly (asynchronous work), so data isn’t lost during spikes or temporary outages and can be retried safely], and data pipeline readiness (where applicable).  
