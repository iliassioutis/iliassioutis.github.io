  ...
  - **Example (what “logged + triaged + pass/fail + approval” means in practice):**
    ...
    - **Triage (how decisions are made):** A recurring triage touchpoint (e.g., daily / 3× weekly) assigns severity (P0/P1/P2/P3), owner, and a decision: fix now, defer with rationale, or not a bug / training.
      - Severity levels (example):
        ...
        - P1 (High): major impact; key functionality is degraded or introduces high risk; a workaround may exist but it’s unacceptable for release unless explicitly approved
    ...
    - **Final approval (who signs off + what evidence):** The designated business owner (and, if applicable, the client representative) provides formal UAT sign-off (email or sign-off document) confirming readiness for release, including any known issues accepted and their planned fix release.
      - Evidence examples: a short UAT summary showing what was tested and the result (e.g., “45 scenarios run, 43 passed, 2 minor issues”), the list of issues found, and a clear “approved to release” confirmation from the business owner.
      - For privacy-sensitive steps, the delivery team keeps simple proof that the feature worked correctly — for example:
        - a screenshot showing the consent prompt was displayed, and
        - a log entry showing the user tapped “I agree” (date/time + which consent it was).
        - if the flow involves viewing sensitive data (e.g., a clinician opening a patient record), a log entry shows who accessed it and when.
- <span style="color:#1f7a6d; font-weight:700;">Test evidence (what we keep as proof)</span>  
  The concrete “proof” that testing happened and what the outcome was — and where it is stored. For example:
  - the list of test cases that were executed (what was tested)
  - the result for each test (pass/fail) and any notes
  - the list of defects found (with severity and status)
  - supporting proof when useful (screenshots, short screen recordings, or relevant log snippets)
  - a short test summary (“X tests run, Y passed, Z open issues”) used for go/no-go
...
SDK v5.3 is pending: our team reviews the vendor changes and re-tests pairing + measurement + save + sync (including disconnect/offline) before approving it for production.”
  ...
  - **Platform constraints (mobile + app stores):**  
    The platform rules that can restrict what we can build and what we can safely release. I document these early so we don’t discover them late in testing or store review.
    - **Supported OS versions:** we publish a minimum supported iOS/Android version and keep it consistent with testing coverage.  
    - **Store policy alignment:** what the app does (permissions + data use) must match what we declare publicly (privacy policy + store privacy section).  
    - **Phone permissions:** which permissions are required for key features, and what the app does if the user denies them (clear message + safe fallback).
