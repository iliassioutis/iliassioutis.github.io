...
- **Reproducibility (where possible)**  
  Make the evidence **repeatable** — so the same input data produces the same results, and someone can follow the steps from raw data → metrics → charts/tables.

  - **Keep an analysis “package” that someone else could rerun (even internally):**  
    - **Analysis scripts / notebooks (or a pipeline):** the step-by-step instructions that calculate metrics and generate outputs (so results aren’t “manual” or dependent on memory).  
    - **Dataset structure + data dictionary:** a short guide that explains:  
      - **What files exist and why:** e.g., `participants.csv`, `measurements.csv`, `reference_device.csv`, `sessions.csv`  
      - **What each column means:** e.g., `spo2_value`, `bp_systolic`, `device_model`, `timestamp`, `participant_id`  
      - **Units and formats:** %, bpm, mmHg, °C; date/time format; numeric precision  
      - **Allowed values / labels:** e.g., source = `AI` / `Device` / `Manual`; status = `valid` / `failed`  
      - **How “bad” data is handled:** what you do if a value is missing, duplicated, out-of-range, or clearly incorrect (exclude, flag, correct, or keep with a note).  
      *Even if the raw data cannot be shared publicly, this guide explains what the dataset looks like and how it is interpreted.*

    - **Method summary (how results are calculated):** a plain-language description of:  
      - **Which metrics are computed:** e.g., agreement/error measures, reliability measures, pass/fail thresholds where relevant  
      - **How results are grouped and summarized:**  
        - per **measurement** (each reading)  
        - per **session** (a set of readings taken together)  
        - per **participant** (aggregating sessions for one person)  
        - overall summary (aggregating across participants)  
      - **Which data is included/excluded and why:** clear rules such as “exclude incomplete pairs”, “exclude readings taken outside protocol window”, “exclude device errors”, “exclude out-of-range values that indicate a failed measurement”.

    - **Chart/table provenance (how each figure is produced):**  
      For every chart/table, keep a simple note like:  
      - “Figure 3 comes from `analysis_agreement.py` using `measurements.csv` and `reference_device.csv` with filters A/B/C.”  
      This makes it possible to regenerate the same figure later and explains exactly which data and rules were used.

  - **Separate experimentation from claim-supporting evidence (so results are defensible):**  
    - **Development experiments:** exploratory work to improve the feature (trying different approaches, tuning settings, testing ideas, informal checks). These results guide development but are **not** the final evidence package.  
    - **Validation evidence used for claims:** the “official” evaluation used to support user-facing statements. Here **the scope is locked** — meaning you fix what is being tested (feature/version), how it is tested (protocol), and what data is included (rules) *before* analyzing results, so you don’t keep changing the method until the numbers look better. This evidence includes documented results **and** documented limitations.
